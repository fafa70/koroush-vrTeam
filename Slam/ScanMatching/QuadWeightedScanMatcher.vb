...
Public Class QuadWeightedScanMatcher
    ...
    Public Overrides Function Match(ByVal manifold As Manifold, ByVal patch1 As Patch, ByVal patch2 As Patch, ByVal seed As Pose2D) As MatchResult
        ...
        Dim curpairs() As Correlation(Of Vector2WithCovariance) = Nothing
            ...
            If curpairs.Length > 0 Then
                ...
                For i As Integer = 0 To curpairs.Length - 1
                    With curpairs(i)
                        ...
                        'rotate covariance matrixes to local coord frame
                        Dim cov1 As Matrix2 = .Point1.Covariance

                        Dim cov2 As New Matrix2
                        With .Point2.Covariance
                            cov2(0, 0) = c ^ 2 * .Item(0, 0) + s ^ 2 * .Item(1, 1) - 2 * c * s * .Item(0, 1)
                            cov2(0, 1) = c * s * (.Item(0, 0) - .Item(1, 1)) + (c ^ 2 - s ^ 2) * .Item(0, 1)
                            cov2(1, 0) = cov2(0, 1)
                            cov2(1, 1) = s ^ 2 * .Item(0, 0) + c ^ 2 * .Item(1, 1) + 2 * c * s * .Item(0, 1)
                        End With
    ...
    Protected Overloads Function CorrelatePointsGlobally(ByVal manifold As Manifold, ByVal patch As Patch) As Correlation(Of Vector2WithCovariance)()

        Dim maxdist As Double = MAX_CORRELATIONDISTANCE

        Dim qtree1 As ManifoldIndex = manifold.ManifoldIndex

        Dim qtree2 As New QuadTree(Of Vector2WithCovariance)
        Dim points2() As Vector2 = patch.GlobalPoints
        Dim covariances2() As Matrix2 = patch.Covariances
        Dim filter2() As Boolean = patch.Filter

        For i As Integer = 0 To points2.Length - 1
            If Not filter2(i) Then
                qtree2.Insert(New Vector2WithCovariance(points2(i), covariances2(i)))
            End If
        Next

        Dim correlations As New List(Of Correlation(Of Vector2WithCovariance))

        Dim point1 As Vector2WithCovariance, point2 As Vector2WithCovariance
        For Each point As Vector2 In points2
            ...
            If point2.Equals(point) Then
                'we found a pair
                correlations.Add(New Correlation(Of Vector2WithCovariance)(point1, point2))
            End If
        Next
