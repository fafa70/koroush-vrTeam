...
Public Class CorridorWalk
    ...
    Protected Overrides Sub OnActivated()
        'Console.WriteLine("[RIKNOTE] Agent\Behavior\Motions\CorridorWalk.vb::OnActivated() called")
        MyBase.OnActivated()
        Me._CurrentMovement = Movement.None
    End Sub

    Protected Overrides Sub OnDeActivated()
        'Console.WriteLine("[RIKNOTE] Agent\Behavior\Motions\CorridorWalk.vb::OnDeActivated() called")
            ...
            For i As Integer = 0 To (.Number - 1)
                ...
                If ._TeamPoses.ContainsKey(._TeamMembers(i)) Then
                    If ._TeamMembers(i) = .Name Then
                        'Console.WriteLine("CorridorWalk: calculating distance to myself")
                    Else
    ...
    Protected Overridable Sub ProcessVictimData(ByVal current_data As VictimData)
        ...
        For Each part As VictimData.VictimPart In current_data.Parts
            ...
            If (distToPart < 1000) Then
                ...
                Me.Control.ActivateMotion(MotionType.AvoidVictim, True) 'explicit return, to FrontierExploration (success) or ObstacleAvoidance (failure)
    ...
    Protected Overridable Sub ProcessGroundTruthData(ByVal current_data As GroundTruthData)
        If Abs(NormalizeAngle(current_data.Roll)) > 0.1 OrElse Abs(NormalizeAngle(current_data.Pitch)) > 0.3 Then
            With Me.Control.Agent
                ...
                .Reverse(0.5F)
            End With
            Console.WriteLine(String.Format("[CorridorWalk] - Something: tilting ({0:f2} , {1:f2}) rad -> Retreat", NormalizeAngle(current_data.Roll), NormalizeAngle(current_data.Pitch)))
    ...
    Protected Overridable Sub ProcessLaserRangeData(ByVal sensorName As String, ByVal laser As LaserRangeData)
        ...
        If minDistance = 1.0 Then
            ...
            Me._CurrentMovement = Movement.None
            'Go for a stroll
            Console.WriteLine(String.Format("CorridorWalk: out of the corridor (minDistance = {0} m), no longer following wall", minDistance))
    ...
    Protected Overridable Sub ProcessSonarData(ByVal sonar As SonarData)
        ...
        If Not IsNothing(Me.Control.Agent) AndAlso Me.Control.Agent.RobotModel.ToLower = "airrobot" Then
            ...
            With sonar
                ...
                End If
            End With
        ElseIf Not IsNothing(Me.Control.Agent) AndAlso Me.Control.Agent.RobotModel.ToLower = "nomad" Then
            ...
            With sonar
                If (isRetreating And (minR < 0.2F Or .Rear(2) > 0.6F Or .Rear(6) > 0.6F Or .Rear(2) < 0.15F Or .Rear(6) < 0.15F)) Then
                    ' RIKNOTE: stop retreating
                    isRetreating = False
                    justRetreated = True
                    'this should works for a Nomad: the configuration is nearly the same as the P2AT
                    'with an P2AT, the used angles are ±0.52, for the Nomad ±0.78

                End If
                If ((Not isRetreating) And (minF < 0.2F Or .Front(3) > 0.6F Or .Front(7) > 0.6F Or .Front(3) < 0.15F Or .Front(7) < 0.15F)) Then
                    ' RIKNOTE: start retreating
                    ' RIKNOTE: in C++ clock() returns number of clock ticks elapsed since process start
                    ' but in VB this is problematic (needs the GetTickCount function from kernel32, etc)
                    ' so just convert the current system time to ticks instead (variable is still unused
        ...
        ElseIf Not IsNothing(Me.Control.Agent) AndAlso Me.Control.Agent.RobotModel.ToLower.StartsWith("atrvjr") Then
